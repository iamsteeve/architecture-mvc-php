...
use \Exception;
use \PDO;
use App\Models\Task as ChildTask;
use App\Models\TaskQuery as ChildTaskQuery;
use App\Models\Map\TaskTableMap;
 ...
 * @method     ChildTaskQuery orderByDate($order = Criteria::ASC) Order by the date column
 ...
 * @method     ChildTaskQuery orderByCategoryId($order = Criteria::ASC) Order by the category_id column
 *
 ...
 * @method     ChildTaskQuery groupByDate() Group by the date column
 ...
 * @method     ChildTaskQuery groupByCategoryId() Group by the category_id column
 *
 ...
 *
 * @method     \App\Models\CategoryQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildTask findOneByDate(string $date) Return the first ChildTask filtered by the date column
 ...
 * @method     ChildTask findOneByCategoryId(int $category_id) Return the first ChildTask filtered by the category_id column *
 ...
 * @method     ChildTask requireOneByDate(string $date) Return the first ChildTask filtered by the date column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildTask requireOneByCategoryId(int $category_id) Return the first ChildTask filtered by the category_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildTask[]|ObjectCollection findByDate(string $date) Return ChildTask objects filtered by the date column
 ...
 * @method     ChildTask[]|ObjectCollection findByCategoryId(int $category_id) Return ChildTask objects filtered by the category_id column
...
abstract class TaskQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \App\Models\Base\TaskQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\App\\Models\\Task', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, description, date, priority, status, category_id FROM task WHERE id = :p0';
        try {
    ...
    public function filterByDate($date = null, $comparison = null)
    {
        if (is_array($date)) {
            $useMinMax = false;
            if (isset($date['min'])) {
                $this->addUsingAlias(TaskTableMap::COL_DATE, $date['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($date['max'])) {
                $this->addUsingAlias(TaskTableMap::COL_DATE, $date['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(TaskTableMap::COL_DATE, $date, $comparison);
    }
    ...
    public function filterByCategoryId($categoryId = null, $comparison = null)
    {
        if (is_array($categoryId)) {
            ...
            if (isset($categoryId['min'])) {
                $this->addUsingAlias(TaskTableMap::COL_CATEGORY_ID, $categoryId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($categoryId['max'])) {
                $this->addUsingAlias(TaskTableMap::COL_CATEGORY_ID, $categoryId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(TaskTableMap::COL_CATEGORY_ID, $categoryId, $comparison);
    }
    ...
    public function filterByCategory($category, $comparison = null)
    {
        if ($category instanceof \App\Models\Category) {
            return $this
                ->addUsingAlias(TaskTableMap::COL_CATEGORY_ID, $category->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(TaskTableMap::COL_CATEGORY_ID, $category->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
